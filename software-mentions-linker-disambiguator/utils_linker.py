...
def update_mentions_df_found_package(top_mentions_df, found_mentions_df, package_type):
  """
  Updates an individual metadata df with booleans to note if a mention has been found in a corresponding metadata file

  :param top_mentions_df: df to update
  :param found_mentions_df: metadata_df with linked mentions
  :param package_type: type of package (eg 'pypi, CRAN, bioconductor, etc')
  """
  top_mentions_df.loc[:, 'found_' + package_type] = False
  top_mentions_df.loc[top_mentions_df['software'].isin(found_mentions_df['software_mention'].unique()), 'found_' + package_type] = True

def update_df_with_metadata_bools(df, pypi_df, bioconductor_df, cran_df, scicrunch_df, github_df):
  """
  Updates a set of metadata dfs with booleans to note if a mention has been found in a corresponding metadata file

  :param df: df to update
  :param pypi_df: PyPI metadata df
  :param biconductor_df: Bioconductor metadata df
  :param cran_df: CRAN metadata df
  :param scicrunch_df: Scicrunch metadata df
  :param github_df: Github metadata df
  """
  update_mentions_df_found_package(df, pypi_df, 'pypi')
  update_mentions_df_found_package(df, bioconductor_df, 'bioconductor')
  update_mentions_df_found_package(df, cran_df, 'cran')
  update_mentions_df_found_package(df, scicrunch_df, 'scicrunch')
  update_mentions_df_found_package(df, github_df, 'github')
